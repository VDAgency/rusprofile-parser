...
**Статус (2026-05-20):** все 7 блоков выполнены, в проде с 2026-05-15
(коммит `7f36e4b` + hotfix Telegram Web `9cff9ec`). Следующий этап —
**[Этап 3: Биллинг и тарифы](./roadmap_billing_tariffs.md)**.
...
- [x] **6.1** Sheets — новые колонки (Я.Карты-сигналы, кросс-обогащение, универсальный скоринг, ИИ-блок), сортировка `hot first → ai_score desc`, цветовое форматирование (`hot` → зелёный, `quota_exceeded` → жёлтый, `skip` → серый). Изменить `src/api/export_xlsx.py` и Sheets-pusher.
- [x] **6.2** Mini App: плашка тарифа в шапке, раздел «ИИ-профили» (двухшаговый UX), выпадающий список профилей в форме парсинга (для AI-тарифа), чекбокс «Кросс-обогащение источников» (для всех).
- [x] **6.3** Карточки запусков в истории — отображение метрик квалификации (для AI и Simple отдельно), кнопка «Запустить квалификацию».
- [x] **6.4** Smoke-test через бот → commit `feat(ui): тариф, профили, квалификация в Mini App + Sheets-форматирование` → push.

## Блок 7. Документация и приёмка

- [x] **7.1** `docs/qualify_pipeline.md` — короткий гид для клиента: что включает каждый тариф, как создать профиль, как читать колонки в Sheets.
- [x] **7.2** Запуск всех тестов, финальный smoke-test, commit `docs(ai): гид клиента по квалификации` → push.
